...
tpmerc_list = [["010", "VISTA"], ["012", "EXERCÍCIO DE OPÇÕES DE COMPRA"], 
    ["013", "EXERCÍCIO DE OPÇÕES DE VENDA"], ["017", "LEILÃO"], ["020", "FRACIONÁRIO"],
    ["030", "TERMO"], ["050", "FUTURO COM RETENÇÃO DE GANHO"], ["060", "FUTURO COM MOVIMENTAÇÃO CONTÍNUA"],
    ["070", "OPÇÕES DE COMPRA"],["080", "OPÇÕES DE VENDA"]]

indopc_list = [["1", "US$", "CORREÇÃO PELA TAXA DO DÓLAR"],
...
def insert_indopc(element):
    print(element)
    cursor = mydb.cursor(buffered=True)
    query = ("INSERT INTO cotacoes_Bovespa.INDOPC (Code_Number, Code_Text, Description) VALUES (%s,%s,%s)")
    cursor.execute(query, (element[0], element[1], element[2]))
    mydb.commit()
    return

def insert_especi(element):
    cursor = mydb.cursor(buffered=True)
    query = ("INSERT INTO cotacoes_Bovespa.ESPECI (Code,  Description) VALUES (%s,%s)")
    cursor.execute(query, (element[0], element[1]))
    mydb.commit()
    return

def insert_codbi(element):
    cursor = mydb.cursor(buffered=True)
    query = ("INSERT INTO cotacoes_Bovespa.CODBDI (Code,  Description) VALUES (%s,%s)")
    cursor.execute(query, (element[0], element[1]))
    mydb.commit()
    return
